...
use map_color::{ColorType, Record, State};
...
fn build_base_color_id_map() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for result in rdr.deserialize() {
        ...
        if let Some(caps) = block_state_regex.captures(&record.block) {
            ...
            match block_state_color_map
                .entry(block)
                .or_insert(ColorType::Multiple(HashMap::new()))
            {
            ...
            block_state_color_map.insert(
                block,
                ColorType::Single(record.color_id.parse().expect("Expeceted i8")),
            );
        }
    }
